...
namespace Angine {
	...
	void SpriteFont::init(const char* font, int size, char cs, char ce) {
		...
		p_glyphs = new CharGlyph[_regLength + 1];
		for (i = 0; i < _regLength; i++) {
			p_glyphs[i].character = (char)(cs + i);
			std::cout << p_glyphs[i].character << std::endl;
		...
		p_glyphs[_regLength].character = ' ';
		p_glyphs[_regLength].size = p_glyphs[0].size;
		p_glyphs[_regLength].uvRect = glm::vec4(0, 0, (float)rs / (float)bestWidth, (float)rs / (float)bestHeight);
	...
	void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling,
		...
		for (int si = 0; s[si] != 0; si++) {
			...
			if (s[si] == '\n') {
				...
				//if (c == '\u00E4') std::cout << p_glyphs[gi].uvRect.x << " " << p_glyphs[gi].uvRect.y << " " << p_glyphs[gi].uvRect.z << " " << p_glyphs[gi].uvRect.w << std::endl;
