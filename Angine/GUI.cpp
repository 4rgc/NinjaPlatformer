...
void GUI::Init(const std::string& resourceDir) {
	...
	p_context = &CEGUI::System::getSingleton().createGUIContext(p_renderer->getDefaultRenderTarget());
	p_root = CEGUI::WindowManager::getSingleton().createWindow("DefaultWindow", "root");
	p_context->setRootWindow(p_root);
}

void GUI::Destroy() {
	CEGUI::System::getSingleton().destroyGUIContext(*p_context);
}

void GUI::Draw() {
	p_renderer->beginRendering();
	p_context->draw();
	p_renderer->endRendering();
...
void GUI::SetFont(const std::string& fontName) {
	...
	p_context->setDefaultFont(fontName);
}
...
void GUI::OnSDLEvent(SDL_Event & evnt) {
	...
	case SDL_KEYDOWN:
		...
		break;
	case SDL_KEYUP:
		p_context->injectKeyUp(SDLKeyToCEGUIKey(evnt.key.keysym.sym));
		break;
	case SDL_TEXTINPUT:
		codePoint = 0;
		...
		p_context->injectChar(codePoint);
		break;
	case SDL_MOUSEBUTTONDOWN:
		p_context->injectMouseButtonDown(SDLButtonToCEGUIButton(evnt.button.button));
		break;
	case SDL_MOUSEBUTTONUP:
		p_context->injectMouseButtonUp(SDLButtonToCEGUIButton(evnt.button.button));
		break;
	}
}
...
void GUI::Update() {
	...
	p_context->injectTimePulse((float)elapsed / 1000.0f);
}